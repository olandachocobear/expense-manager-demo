    ...
    del([buildDirectory])
      .then(() => {
        ...
        let dependenciesStream = polymerProject.dependencies()
          ...
          //.pipe(uglify()) // Install gulp-uglify to use
      ...
      // })
      .then(() => {
        console.log('CSP prooof-ing...');
        gulp.src(buildDirectory+"/src/*.html")
      //   .pipe(vulcanize({
      //     abspath: '',
      //     excludes: [],
      //     stripExcludes: false,
      ...
      // }))
          ...
          .pipe(gulp.dest(buildDirectory+"/newSrc/"));
      })
      .then(() => {
        console.log('Concat-ing...');
        gulp.src(buildDirectory+"/newSrc/*.js")
          .pipe(concat('all.min.js'))
          // .pipe(uglify())
          .pipe(gulp.dest(buildDirectory+"/src/js"));
      })
      .then(() => {
        console.log('Uglify-ing...');
        gulp.src(buildDirectory+"/src/*.js")
          // .pipe(concat('all.min.js'))
          .pipe(uglify())
          .pipe(gulp.dest(buildDirectory+"/src2/js"));
      })
      .then(() => {
